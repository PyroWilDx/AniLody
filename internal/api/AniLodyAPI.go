	...
	for i := 0; i < len(userIds); i += perPage {
		...
		for _, anime := range userAnimes {
			for _, animeTheme := range anime.AnimeThemes {
				if (isOp(animeTheme.Slug) && !userSettings.IncOp) || (isEd(animeTheme.Slug) && !userSettings.IncEd) {
					continue
				}

				for _, animeThemeEntry := range animeTheme.AnimeThemeEntries {
					if animeThemeEntry.Version > 1 {
						break
					}
...
func getSlug(aSlug string) string {
	aSlug = strings.Replace(aSlug, "OP", "Op", 1)
	aSlug = strings.Replace(aSlug, "ED", "Ed", 1)
	return aSlug
}
